...
class FileLoggingTree(private val context: Context) : Timber.Tree() {
    ...
    init {
        try {
            ...
            WalletLogger.e(TAG, e, "Could not create log file")
        }
    }
    ...
    private fun rotateLog(cacheDir: File, cacheFile: File) {
        runBlocking(Dispatchers.IO) {
            try {
                ...
                WalletLogger.e(TAG, e, "Could not rotate file")
            } catch (e: FileNotFoundException) {
                WalletLogger.e(TAG, e, "Could not find log file")
            }
        }
    }
        ...
        .joinToString(separator = "/") {
            if (idRegex.containsMatchIn(it)) {
                try {
                    ...
                    WalletLogger.w(TAG, e, "Could not sanitise id")
                    it
                }
            } else {
                it

...
import 'paper_wallet_test.mocks.dart';
...
void main() {
  ...
  testSnapshot('paper wallet mobile', (tester) async {
    ...
    when(viewModel.privateKeyController).thenAnswer((_) => TextEditingController(text: "mock private key"));
    when(viewModel.privateKeyFocusNode).thenAnswer((_) => FocusNode());
    when(viewModel.importedError).thenAnswer((_) => "Invalid private key. Please try again.");
    when(viewModel.coordinator)
        .thenAnswer((_) => MockPaperWalletCoordinator());
